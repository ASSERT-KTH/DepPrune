jsonlist=$(jq -r '.projects' "repos_copy1_1.json")
...
do
    ...
    for line in "${lines[@]}"; do
        ...
        if [[ $command_output == *$testPassSignal* ]]; then
            echo $projectName","$line",1" >> ../../test_result_output1_1.txt
        else
            echo $projectName","$line",0" >> ../../test_result_output_error1_1.txt
        fi
        cd ..
        rm -rf $projectName
        cd ..
