jsonlist=$(jq -r '.projects' "repos_copy7.json")
...
do
    ...
    for line in "${lines[@]}"; do
        ...
        if [[ $command_output == *$testPassSignal* ]]; then
            echo $projectName","$line",1" >> ../../test_passed_result_output7.txt
        else
            echo $projectName","$line",0" >> ../../test_passed_result_output_error7.txt
        fi
        cd ..
        rm -rf $projectName
        cd ..
