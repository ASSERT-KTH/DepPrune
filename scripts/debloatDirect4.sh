jsonlist=$(jq -r '.projects' "repos_copy4.json")
...
do
    ...
    for line in "${lines[@]}"; do
        ...
        if [[ $command_output == *$testPassSignal* ]]; then
            echo $projectName","$line",1" >> ../../test_direct_result_output4.txt
        else
            echo $projectName","$line",0" >> ../../test_direct_result_output_error4.txt
        fi
        cd ..
        rm -rf $projectName
        cd ..
