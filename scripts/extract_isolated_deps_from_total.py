...
filePath = f'./Playground/{project}/dependent-files-total.json'
f_package = open(filePath, encoding="utf-8")  
clientsDict = json.load(f_package)

deps_path = f'./Playground/{project}/total__deps.txt'
with open(deps_path) as f:
    potential_deps = f.read().splitlines()

old_path_pre = f"/data/js-variants/multee/Playground/{project}/" 

client_files_path = f"./Playground/{project}/non-isolated-clients_from_total.json"
non_isolated_json = {}
...
isolated_file = open(f"./Playground/{project}/isolated-deps_from_total.txt", "a")
non_isolated_file = open(f"./Playground/{project}/non-isolated-deps_from_toal.txt", "a")
