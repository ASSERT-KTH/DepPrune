jsonlist=$(jq -r '.projects' "repos_copy2.json")
...
do
    ...
    for line in "${lines[@]}"; do
        ...
        if [[ $command_output == *$testPassSignal* ]]; then
            echo $projectName","$line",1" >> ../../test_direct_result_output2.txt
        else
            echo $projectName","$line",0" >> ../../test_direct_result_output_error2.txt
        fi
        cd ..
        rm -rf $projectName
        cd ..
