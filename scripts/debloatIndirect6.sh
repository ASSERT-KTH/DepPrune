jsonlist=$(jq -r '.projects' "repos_copy3_3.json")
...
do
    ...
    for line in "${lines[@]}"; do
        ...
        if [[ $command_output == *$testPassSignal* ]]; then
            echo $projectName","$line",1" >> ../../test_result_output3_3.txt
        else
            echo $projectName","$line",0" >> ../../test_result_output_error3_3.txt
        fi
        cd ..
        rm -rf $projectName
        cd ..
