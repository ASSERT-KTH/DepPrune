jsonlist=$(jq -r '.projects' "repos_copy6.json")
...
do
    ...
    for line in "${lines[@]}"; do
        ...
        if [[ $command_output == *$testPassSignal* ]]; then
            echo $projectName","$line",1" >> ../../test_direct_result_output6.txt
        else
            echo $projectName","$line",0" >> ../../test_direct_result_output_error6.txt
        fi
        cd ..
        rm -rf $projectName
        cd ..
